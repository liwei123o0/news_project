...
import scrapy
from basespider.items import SougouSpiderItem
from scrapy.spiders import Spider


class SogouWXSpider(Spider):
    name = 'sogou_wx'
    proxy = False
    quality = 1
    debug = False
    ...
    url = u"http://weixin.sogou.com/weixin?type=2&ie=utf8&query={keyword}&tsn=1&ft=&et=&interation=&wxid=&usip="
    keywords = [u"716所", u"73071部队", u"JXUST", u"P3芃w#九淼=", u"七一六", u"万安", u"万年县", u"万载", u"三山区", u"三房巷", u"上上电缆",
                ...
                u"龙南", u"龙口", u"龙子湖"]
    start_urls = []
    random.sample(keywords, len(keywords))
    for keyword in keywords:
        start_urls.append(url.format(keyword=keyword))

    def parse(self, response):
        sel = scrapy.Selector(response)
        items = []
        for divlist in sel.xpath("//div[@class='txt-box']"):
            item = SougouSpiderItem()
            item['url'] = divlist.xpath("./h3/a/@href").extract_first()
            item['pubtime'] = divlist.xpath("./div/@t").extract_first()
            items.append(item)
        for item in items:
            if item['url'] != None:
                yield scrapy.Request(item['url'], meta={'item': item}, callback=self.parse_item)
    ...
    def parse_item(self, response):
        ...
        item[u'url_md5'] = url_md5
        item['title'] = "".join(sel.xpath("//h2/text()").extract()).lstrip().rstrip()
        item['content'] = "".join(sel.xpath("//div[@id='js_content']//text()").extract()).lstrip().rstrip()
        item['author'] = "".join(sel.xpath("//a[@id='js_name']/text()").extract()).lstrip().rstrip()
