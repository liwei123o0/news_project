...
def item_fileds(item, tablename, type_name, debug):
    ...
    if debug:
        ...
        conn = MySQLdb.connect(host="172.16.100.217", port=8635, user='acq_data', passwd='b2aO6CxVpcWRKF!3',
                               db='acq_data',
                               charset=u"utf8")
        cur = conn.cursor()
        for k, v in item.iteritems():
    ...
    for html in htmls:
        ...
        for d in dom:
            ...
            item['re_author'] = u"".join(
                d.xpath(json.loads(name_spider['fields'])['fields']['re_author']['xpath'])).strip()
